...
MANIFEST_URL = (
    "https://raw.githubusercontent.com/GD-fandev/Godimap/"
    "main/update/manifest.json"
)
RELEASE_URL_PREFIX = "https://github.com/GD-fandev/Godimap/releases/download/"
...
def validate_manifest(payload):
    ...
    if not asset_url.startswith(RELEASE_URL_PREFIX):
        raise UpdateError("The update URL is not an allowed GODIMAP release URL.")
...
def download_and_install(manifest, app_dir, progress_callback=None):
    ...
    if not app_dir.is_dir():
        raise UpdateError("The GODIMAP application folder does not exist.")
    operation_id = uuid.uuid4().hex
    stage_dir = app_dir / f".godimap-update-stage-{operation_id}"
    backup_dir = app_dir / f".godimap-update-backup-{operation_id}"
...
def build_update_archive(project_dir, output_path, version):
    ...
    return {
        "schema": 1,
        "version": version,
        "asset_url": f"{RELEASE_URL_PREFIX}{tag}/godimap-mapdata.zip",
        "sha256": digest,
