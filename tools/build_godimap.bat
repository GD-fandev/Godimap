...
call ".venv\Scripts\python.exe" -m PyInstaller --noconfirm --clean --distpath "%OUTPUT_DIR%" "packaging\GODIMAP.spec"
...
echo GODIMAP build and distribution assembly completed: %OUTPUT_DIR%
exit /b 0

:failed
echo GODIMAP build failed.
pause
exit /b 1
